...
public class GTRecipe implements net.minecraft.world.item.crafting.Recipe<Container> {
    ...
    public ActionResult matchRecipeContents(IO io, IRecipeCapabilityHolder holder, Map<RecipeCapability<?>, List<Content>> contents, boolean isTick) {
        ...
        for (Map.Entry<RecipeCapability<?>, List<Content>> entry : contents.entrySet()) {
            ...
            if (result.result().content != null || !result.result().slots.isEmpty()) {
                if (io == IO.IN) {
                    return ActionResult.fail(() -> Component.translatable("gtceu.recipe_logic.insufficient_in").append(": ").append(result.capability().getName()), 0.0F);
                } else if (io == IO.OUT) {
                    return ActionResult.fail(() -> Component.translatable("gtceu.recipe_logic.insufficient_out").append(": ").append(result.capability().getName()), 0.0F);
                } else {
    ...
    public Map<RecipeCapability<?>, List<Content>> doTrim(Map<RecipeCapability<?>, List<Content>> current, Map<RecipeCapability<?>, Integer> trimLimits) {
        ...
        for (Map.Entry<RecipeCapability<?>, Integer> entry : trimLimits.entrySet()) {
            ...
            if (nonChanced.size() >= outputLimit) {
            ...
            if (!nonChanced.isEmpty() && (nonChanced.size() + chanced.size()) >= outputLimit) {
            ...
            if (!chanced.isEmpty()) outputs.computeIfAbsent(key, $ -> new ArrayList<>()).addAll(chanced.stream().map(cont -> cont.copy(key, null)).toList());
            trimmed.add(key);
        }
    ...
    public static record ActionResult(boolean isSuccess, @Nullable Supplier<Component> reason, float expectingRate) {
        public static final ActionResult SUCCESS = new ActionResult(true, null, 0);
        public static final ActionResult FAIL_NO_REASON = new ActionResult(true, null, 0);
