...
public class OreGenCache {
    ...
    private final Cache<ChunkPos, List<GeneratedVeinMetadata>> veinMetadataByOrigin = CacheBuilder.newBuilder().maximumSize(veinMetadataCacheSize).softValues().build();
    private final Cache<ChunkPos, List<GeneratedVein>> generatedVeinsByOrigin = CacheBuilder.newBuilder().maximumSize(oreGenerationCacheSize).softValues().build();
    private final Cache<ChunkPos, List<GeneratedIndicators>> indicatorsByOrigin = CacheBuilder.newBuilder().maximumSize(oreIndicatorCacheSize).softValues().build();
