...
public class FusionReactorMachine extends WorkableElectricMultiblockMachine implements ITieredMachine {
    ...
    public static GTRecipe recipeModifier(MetaMachine machine, @NotNull GTRecipe recipe, @NotNull OCParams params, @NotNull OCResult result) {
        if (machine instanceof FusionReactorMachine fusionReactorMachine) {
            ...
            if (heatDiff <= 0) {
                return RecipeHelper.applyOverclock(new OverclockingLogic(PERFECT_HALF_DURATION_FACTOR, PERFECT_HALF_VOLTAGE_FACTOR, false), recipe, fusionReactorMachine.getMaxVoltage(), params, result);
            }
    ...
    public boolean onWorking() {
        ...
        if (color == -1) {
            if (!recipe.getOutputContents(FluidRecipeCapability.CAP).isEmpty()) {
                int newColor = -16777216 | FluidHelper.getColor(FluidRecipeCapability.CAP.of(recipe.getOutputContents(FluidRecipeCapability.CAP).get(0).getContent()).getStacks()[0]);
