...
public class GTOreVeinWidget extends WidgetGroup {
    ...
    private void setupBaseGui(BedrockFluidDefinition fluid) {
        ...
        TankWidget fluidSlot = new TankWidget(new FluidStorage(FluidStack.create(storedFluid, 1000)), 51, 18, false, false);
    ...
    private void setupDimensionMarker(int yPosition) {
        if (this.dimensionFilter != null) {
            ...
            for (int i = 0; i < dimMarkers.length; i++) {
                ...
                SlotWidget dimSlot = new SlotWidget(transfer, i, 5 + (16 + interval) * (i - row * rowSlots), yPosition + 18 * row, false, false).setIngredientIO(IngredientIO.CATALYST);
    ...
    public static List<ItemStack> getContainedOresAndBlocks(GTOreDefinition oreDefinition) {
        return oreDefinition.veinGenerator().getAllEntries().stream().flatMap(entry -> entry.getKey().map(state -> Stream.of(state.getBlock().asItem().getDefaultInstance()), material -> {
    ...
    public static List<ItemStack> getRawMaterialList(GTOreDefinition oreDefinition) {
        return oreDefinition.veinGenerator().getAllEntries().stream().map(entry -> entry.getKey().map(state -> state.getBlock().asItem().getDefaultInstance(), material -> ChemicalHelper.get(TagPrefix.rawOre, material))).toList();
    }

    public static List<ItemStack> getRawMaterialList(BedrockOreDefinition bedrockOreDefinition) {
        return bedrockOreDefinition.materials().stream().map(entry -> ChemicalHelper.get(TagPrefix.rawOre, entry.getFirst())).toList();
    }
