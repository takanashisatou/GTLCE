...
public class ElectricGearBoxMachine extends TieredEnergyMachine implements IKineticMachine, IFancyUIMachine {
    ...
    public float getRotationSpeedModifier(Direction direction) {
        if (direction == getRotationFacing().getOpposite()) return -1;
        return 1;
    }
    ...
    public Widget createUIWidget() {
        ...
        group.addWidget(new ImageWidget(36, 4, 92, 20, new TextTexture("").setWidth(92).setType(TextTexture.TextType.ROLL).setSupplier(() -> "Speed: " + getKineticHolder().workingSpeed))).addWidget(new ButtonWidget(4, 24, 30, 20, new GuiTextureGroup(GuiTextures.VANILLA_BUTTON, new TextTexture("-8rpm")), cd -> {
            if (!cd.isRemote) {
                int amount = cd.isCtrlClick ? cd.isShiftClick ? 32 : 16 : cd.isShiftClick ? 4 : 1;
                setCurrentAmps(currentAmps - amount);
            }
        }).setHoverTooltips("gui.widget.incrementButton.default_tooltip")).addWidget(new ButtonWidget(130, 24, 30, 20, new GuiTextureGroup(GuiTextures.VANILLA_BUTTON, new TextTexture("+8rpm")), cd -> {
            if (!cd.isRemote) {
                int amount = cd.isCtrlClick ? cd.isShiftClick ? 32 : 16 : cd.isShiftClick ? 4 : 1;
                setCurrentAmps(currentAmps + amount);
            }
        }).setHoverTooltips("gui.widget.incrementButton.default_tooltip")).addWidget(new ImageWidget(36, 24, 92, 20, new GuiTextureGroup(new ColorRectTexture(-16777216), new ColorBorderTexture(1, -1), new TextTexture("").setWidth(92).setType(TextTexture.TextType.ROLL).setSupplier(() -> getCurrentRPM() + "rpm"))));
