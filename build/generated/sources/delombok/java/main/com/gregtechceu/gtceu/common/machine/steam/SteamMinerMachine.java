...
public class SteamMinerMachine extends SteamWorkableMachine implements IMiner, IControllable, IExhaustVentMachine, IUIMachine, IMachineLife, IDataInfoProvider {
    ...
    public ModularUI createUI(Player entityPlayer) {
        ...
        ModularUI builder = new ModularUI(175, 176, this, entityPlayer).background(GuiTextures.BACKGROUND_STEAM.get(false));
        ...
        for (int y = 0; y < rowSize; y++) {
            for (int x = 0; x < rowSize; x++) {
                ...
                builder.widget(new SlotWidget(exportItems, index, 142 - rowSize * 9 + x * 18, 18 + y * 18, true, false).setBackgroundTexture(GuiTextures.SLOT_STEAM.get(false)));
            }
        }
        builder.widget(new LabelWidget(5, 5, getBlockState().getBlock().getDescriptionId()));
        builder.widget(new PredicatedImageWidget(79, 42, 18, 18, GuiTextures.INDICATOR_NO_STEAM.get(isHighPressure)).setPredicate(recipeLogic::isWaiting));
    ...
    void addDisplayText(List<Component> textList) {
        ...
        if (this.getRecipeLogic().isDone()) textList.add(Component.translatable("gtceu.multiblock.large_miner.done").setStyle(Style.EMPTY.withColor(ChatFormatting.GREEN)));
         else if (this.getRecipeLogic().isWorking()) textList.add(Component.translatable("gtceu.multiblock.large_miner.working").setStyle(Style.EMPTY.withColor(ChatFormatting.GOLD)));
    ...
    public boolean drainInput(boolean simulate) {
        ...
        if (!this.isVentingBlocked() && resultSteam >= 0L && resultSteam <= steamTank.getTankCapacity(0)) {
            if (!simulate) steamTank.drainInternal(energyPerTick, false);
            return true;
        }
        return false;
    }
