        ...
        for (ItemStack outputStack : outputs) {
            if (ChemicalHelper.getPrefix(outputStack.getItem()) == TagPrefix.crushed) {
                if (getDropCountMultiplier() > 0) {
                    ...
                    outputStack = ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE).build().apply(outputStack, new LootContext.Builder(builder.withParameter(LootContextParams.TOOL, fortunePick).create(LootContextParamSets.BLOCK)).create(null));
                }
            }
