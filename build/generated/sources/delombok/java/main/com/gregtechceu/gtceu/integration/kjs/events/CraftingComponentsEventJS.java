...
public class CraftingComponentsEventJS extends StartupEventJS {
    ...
    public void modify(CraftingComponent.Component component, Map<Number, Object> map) {
        Map<Integer, Object> newMap = map.entrySet().stream().map(entry -> Map.entry(entry.getKey().intValue(), entry.getValue())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    ...
    public void modifyTag(CraftingComponent.Component component, Map<Number, ResourceLocation> map) {
        Map<Integer, Object> newMap = map.entrySet().stream().map(entry -> Map.entry(entry.getKey().intValue(), TagKey.create(Registries.ITEM, entry.getValue()))).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        component.appendIngredients(newMap);
    }
