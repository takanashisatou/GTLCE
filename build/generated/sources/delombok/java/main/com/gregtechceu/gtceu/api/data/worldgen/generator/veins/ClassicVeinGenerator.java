...
public class ClassicVeinGenerator extends VeinGenerator {
    ...
    public List<Map.Entry<Either<BlockState, Material>, Integer>> getAllEntries() {
        ...
        primary.target.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, primary.layers / totalWeight)));
        secondary.target.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, secondary.layers / totalWeight)));
        between.target.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, between.layers / totalWeight)));
        sporadic.target.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, 1)));
        return result;
    }
    ...
    public VeinGenerator build() {
        ...
        Preconditions.checkArgument(primary.layers + secondary.layers >= between.layers, "Error: cannot be more \"between\" layers than primary and secondary layers combined!");
