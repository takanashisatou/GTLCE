...
public class PumpCover extends CoverBehavior implements IUICover, IControllable {
    ...
    protected Map<FluidStack, Long> enumerateDistinctFluids(IFluidTransfer fluidTransfer, TransferDirection direction) {
        ...
        for (int tank = 0; tank < fluidTransfer.getTanks(); tank++) {
            if (!canTransfer(fluidTransfer, direction, tank)) continue;
    ...
    public Widget createUIWidget() {
        ...
        transferRateWidget = new LongInputWidget(10, 20, 134, 20, this::getCurrentBucketModeTransferRate, this::setCurrentBucketModeTransferRate).setMin(0L);
        setBucketMode(this.bucketMode); // initial input widget config happens here
        group.addWidget(transferRateWidget);
        group.addWidget(new EnumSelectorWidget<>(146, 20, 20, 20, Arrays.stream(BucketMode.values()).filter(m -> m.multiplier <= maxMilliBucketsPerTick).toList(), bucketMode, this::setBucketMode).setTooltipSupplier(this::getBucketModeTooltip));
