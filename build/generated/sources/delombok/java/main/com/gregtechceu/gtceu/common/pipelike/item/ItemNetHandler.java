...
public class ItemNetHandler implements IItemTransfer {
    ...
    public static int countStack(IItemTransfer handler, ItemStack stack, RobotArmCover arm, boolean isStackSpecific) {
        ...
        for (int i = 0; i < handler.getSlots(); i++) {
            ...
            if (isStackSpecific ? ItemStackHashStrategy.comparingAllButCount().equals(stack, slot) : arm.getFilterHandler().getFilter().test(slot)) {
                count += slot.getCount();
            }
        }
        return count;
    ...
    private void transferTo(ItemRoutePath handler, boolean simulate, int amount) {
        if (simulate) simulatedTransfersGlobalRoundRobin.merge(handler.toFacingPos(), amount, Integer::sum);
         else pipe.getTransferred().merge(handler.toFacingPos(), amount, Integer::sum);
    }
    ...
    private int didTransferTo(ItemRoutePath handler, boolean simulate) {
        if (simulate) return simulatedTransfersGlobalRoundRobin.getOrDefault(handler.toFacingPos(), 0);
        return pipe.getTransferred().getOrDefault(handler.toFacingPos(), 0);
    }

    private void resetTransferred(boolean simulated) {
        if (simulated) simulatedTransfersGlobalRoundRobin.clear();
         else pipe.resetTransferred();
    }
