...
public class CuboidVeinGenerator extends VeinGenerator {
    ...
    public List<Map.Entry<Either<BlockState, Material>, Integer>> getAllEntries() {
        ...
        top.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, 2)));
        middle.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, 3)));
        bottom.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, 4)));
        spread.map(blockStates -> blockStates.stream().map(state -> Either.<BlockState, Material>left(state.state)), material -> Stream.of(Either.<BlockState, Material>right(material))).forEach(entry -> result.add(Map.entry(entry, 7)));
        return result;
    }
    ...
    public CuboidVeinGenerator(final Either<List<OreConfiguration.TargetBlockState>, Material> top, final Either<List<OreConfiguration.TargetBlockState>, Material> middle, final Either<List<OreConfiguration.TargetBlockState>, Material> bottom, final Either<List<OreConfiguration.TargetBlockState>, Material> spread, final int minY, final int maxY) {
        this.top = top;
        this.middle = middle;
        this.bottom = bottom;
        this.spread = spread;
